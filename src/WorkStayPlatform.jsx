...
const getExperienceImage = (exp = {}) => {
  // ID별 실제 이미지 매핑 (1~14번 이미지)
  if (exp.id === 3) return '/1.png';  // 대전 식물원 가드닝 체험
  if (exp.id === 2) return '/2.png';  // 천안 배 농장 수확 체험  
  if (exp.id === 3) return '/3.png';  // 청주 딸기 농장 체험
  if (exp.id === 4) return '/3.png';  // 부산 해안가 염전 체험
  if (exp.id === 5) return '/4.png';  // 제주 감귤밭 일손돕기
  if (exp.id === 6) return '/5.png';  // 강원도 산나물 채취 체험
  if (exp.id === 7) return '/7.png';  // 보령 고구마 캐기 알바(머드 체험 연계)
  if (exp.id === 8) return '/8.png';  // 논산 딸기 수확/가공 알바
  if (exp.id === 9) return '/9.png';  // 옥천 포도 수확 알바
  if (exp.id === 10) return '/10.png'; // 예산 한우 목장 일손 알바
  if (exp.id === 11) return '/11.png'; // 단양 마늘 수확 알바
  if (exp.id === 12) return '/12.png'; // 괴산 유기농 채소 재배 보조 알바
  if (exp.id === 13) return '/13.png'; // 세종 허브 농장 포장·관리 알바
  if (exp.id === 14) return '/14.png'; // 공주 국화밭 정리 알바
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="card p-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">알바 태그</label>
                    <select
                      value={selectedTag}
